...
%
function [ret,cur] = read_file_realtime(filename,curpin)
%% Input check
narginchk(1,2);%判断输入的个数，若是在1～2之间，不执行任何操作，否则报错。
...
status = fseek(f, 0, 'eof');%将文件指针返回至文件尾部
if status ~= 0
    [msg, errno] = ferror(f);
    error('Error %d seeking: %s', errno, msg);%无法返回文件尾报错。
    fclose(f);
    return;
end
len = ftell(f);

status = fseek(f, curpin, 'bof');
if status ~= 0
    [msg, errno] = ferror(f);
...
cur = curpin;                   % Current offset into file
...
while cur < (len - 3)
    % Read size and code
    field_len = fread(f, 1, 'uint16', 0, 'ieee-be');%写入时，先写一个数据的大小，再写实际数据,所以读也要先读大小，然后才是数据。
    code = fread(f,1);
    cur = cur+3;
